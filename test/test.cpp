...
void DisplayInformation(int fs, int nbit, int x_length) {
  ...
  printf("Length %f [sec]\n", static_cast<float>(x_length) / fs);
}

void F0EstimationDio(float *x, int x_length,
...
void ParameterModification(int argc, char *argv[], int fs, int f0_length,
    int fft_size, float *f0, float **spectrogram) {
  // F0 scaling
  if (argc >= 4) {
  ...
  float *freq_axis1 = new float[fft_size];
  float *freq_axis2 = new float[fft_size];
  float *spectrum1 = new float[fft_size];
  float *spectrum2 = new float[fft_size];

  for (int i = 0; i <= fft_size / 2; ++i) {
    freq_axis1[i] = ratio * i / fft_size * fs;
    freq_axis2[i] = static_cast<float>(i) / fft_size * fs;
  }
